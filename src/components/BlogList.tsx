...
const CATEGORIES = [
  ...
  { id: "phong-thuy", label: "🔮 Phong thủy", value: "Phong thủy" },
...
export default function BlogList({ allBlogItems = [] }: BlogListProps) {
  ...
  const savedCount = actualSavedBlogs.length;

  const filteredBlogs = useMemo(() => {
    if (activeTab === "all") return safeBlogs;
    if (activeTab === "saved") return actualSavedBlogs;
              ...
              >
                <div className="relative aspect-[16/10] bg-slate-100 overflow-hidden">
                  <Link href={`/blog/${currentSlug}`} className="absolute inset-0 z-0">
                    <Image src={currentImage} alt={`Ảnh bài viết: ${currentTitle}`} title={currentTitle} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" loading="lazy" decoding="async" />
                  </Link>
